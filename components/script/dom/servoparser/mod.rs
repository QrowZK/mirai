...
impl ParserContext {
    ...
    fn load_document(&mut self, cx: &mut JSContext) {
        ...
        let Some(media_type) = MimeClassifier::get_media_type(&mime_type) else {
            // Mirai: content we cannot render is handed to the embedder as a
            // download request rather than replaced with an error page.
            ...
            parser
                .document
                .window()
                .send_to_embedder(embedder_traits::EmbedderMsg::RequestDownload(
                    self.webview_id,
                    self.url.as_url().clone(),
                    suggested_filename.clone(),
                ));
            let name = suggested_filename.unwrap_or_else(|| self.url.to_string());
            let page = format!(
                "<html><body><p>Downloading <b>{}</b>&hellip; ({})</p></body></html>",
                name, &mime_type,
            );
