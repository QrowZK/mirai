...
pub(crate) struct HTMLMediaElement {
    ...
    /// The `MediaSource` this element is attached to, when its resource is a
    /// Media Source Extensions object URL.
    attached_media_source: MutNullableDom<MediaSource>,
...
impl HTMLMediaElement {
    ...
    fn fetch_request(&self, cx: &JSContext, offset: Option<u64>, seek_lock: Option<SeekLock>) {
        ...
        // Data for a MediaSource attachment arrives via SourceBuffer appends,
        // never via fetch. Grant the pipeline's initial seek to the start of
        // the stream (appended data begins there) and refuse everything else.
        if self.attached_media_source.get().is_some() {
            if let Some(seek_lock) = seek_lock {
                seek_lock.unlock(offset.unwrap_or(0) == 0);
            }
            return;
        }
    ...
    fn resource_fetch_algorithm(&self, cx: &JSContext, resource: Resource) {
        ...
        match resource {
            Resource::Url(url) => {
                // https://w3c.github.io/media-source/#mediasource-attachment
                // If the URL was created by URL.createObjectURL(mediaSource),
                // attach to the MediaSource: data arrives via SourceBuffer
                // appends rather than a network fetch.
                if let Some(media_source) = MediaSource::lookup_url(&url) {
                    self.attached_media_source.set(Some(&media_source));
                    self.network_state.set(NetworkState::Loading);
                    self.queue_media_element_task_to_fire_event(atom!("loadstart"));
                    media_source.attach(self);
                    return;
                }
    ...
    /// Push a chunk of media data appended through Media Source Extensions
    /// into the playback pipeline.
    pub(crate) fn push_media_source_data(&self, data: Vec<u8>) -> Result<(), ()> {
        let player = self.player.borrow();
        let Some(player) = player.as_ref() else {
            return Err(());
        };
        player.lock().unwrap().push_data(data).map_err(|error| {
            warn!("Could not push MSE data to player: {error:?}");
        })
    }

    /// Signal end-of-stream for a Media Source Extensions attachment.
    pub(crate) fn end_media_source_stream(&self) {
        if let Some(player) = self.player.borrow().as_ref() &&
            let Err(error) = player.lock().unwrap().end_of_stream()
        {
            warn!("Could not signal MSE end of stream: {error:?}");
        }
    }
