...
pub trait WebViewDelegate {
    ...
    /// The document at the given URL cannot be rendered by the engine (for
    /// example an unknown content type) and should be downloaded by the
    /// embedder instead.
    fn request_download(&self, _webview: WebView, _url: Url, _suggested_filename: Option<String>) {}
